...
# --- THE FIX IS HERE: A robust and corrected helper function ---
async def edit_or_send_message(ctx, text, reply_markup):
    ...
    # If the message has a photo, we can only edit the caption.
    if message_obj.photo:
        try:
            ...
            # Handle cases where the message is not modified or other errors
            if "message is not modified" not in str(e).lower():
                logger.error(f"Could not edit caption: {e}")
    # If it's a text-only message, we edit the text.
    else:
        try:
            ...
            if "message is not modified" not in str(e).lower():
                logger.error(f"Could not edit text: {e}")
